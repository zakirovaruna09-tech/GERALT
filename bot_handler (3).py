...
import os, threading, json, random, logging
...
def _owner_only(message) -> bool:
    """Строгая проверка владельца — для команд, которые управляют реальным
    ПК/почтой (open/close/steam/email/doc/clear). Чужим — отказ."""
    global _owner_id
    ...
    if message.from_user.id != _owner_id:
        bot.reply_to(message,
                     ...
                     "Демо-возможности: /start /status /top /weather /ai /quote")
        return False
    return True
...
def cmd_start(msg):
    ...
    text = (
        ...
        "Просто напишите текст — отвечу через ИИ.\n"
        "Отправьте фото — проанализирую.\n\n"
        "*Команды владельца ПК (управление локальным компьютером):*\n"
        "/screenshot, /open, /close, /steam, /email, /doc, /clear\n"
    )
...
if __name__ == "__main__":
    ...
    print(f"  [TG]  Токен: {BOT_TOKEN[:10]}...{BOT_TOKEN[-5:] if BOT_TOKEN else '(ПУСТО)'}")
    try:
        me = bot.get_me()
        print(f"  [TG]  Связь с Telegram OK. Бот: @{me.username} (id={me.id})")
    except Exception as e:
        print(f"  [TG]  ОШИБКА связи с Telegram API: {type(e).__name__}: {e}")

    try:
        bot.remove_webhook()
        print("  [TG]  Webhook снят (если был установлен).")
    except Exception as e:
        print(f"  [TG]  Ошибка remove_webhook: {e}")
